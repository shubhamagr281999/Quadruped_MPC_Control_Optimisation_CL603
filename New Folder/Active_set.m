function U  = Active_set(x0,Xref,n_steps,mu,fmin,fmax,Q,R,onGround,Bi,Ai)
    ...
    U=zeros(12,1);
    WorkingSetindices=zeros(size(A,1),1);
    x(:,kk) = xinitial;
    z = A*x(:,kk)-b;
    activeConsCount=0;
    activeCons = find(abs(z)<=0.0000001);
    activeConsCount=size(activeCons,1);
    WorkingSetindices(1:activeConsCount,1)=activeCons(:,1);   
    tW = [1:m];
    while kk<1500        
        row = size(A(WorkingSetindices(1:activeConsCount,:),:),1);
        ...
        if norm(pk)<0.0001 
            ...
            nw =  setdiff(tW,W);
            alph = zeros(size(A,1),1);
            for NonWorkingIndex=1:size(nw,2)
                if A(nw(NonWorkingIndex),:)*pk < 0
                    alph(nw(NonWorkingIndex)) = (b(nw(NonWorkingIndex)) - A(nw(NonWorkingIndex),:)*x(:,kk))/(A(nw(NonWorkingIndex),:)*pk);               
                end
            end
            alph(alph<=0) = 10;
            alphak = min([1,alph]);
